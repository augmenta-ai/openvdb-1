...
/// perspective and orthographic cameras (both designed to mimic a Houdini
/// camera), a Film class and some rather naive shaders.
///
...
inline void rayTrace(const GridT &, const BaseShader &, BaseCamera &,
                     size_t pixelSamples = 1, unsigned int seed = 0,
                     bool threaded = true);

/// @brief Ray-trace a volume using a given ray intersector.
template <typename GridT, typename IntersectorT>
inline void rayTrace(const GridT &, const IntersectorT &, const BaseShader &,
                     BaseCamera &, size_t pixelSamples = 1,
                     unsigned int seed = 0, bool threaded = true);

///////////////////////////////LEVEL SET RAY TRACER
//////////////////////////////////////////

/// @brief A (very) simple multithreaded ray tracer specifically for narrow-band
/// level sets.
/// @details Included primarily as a reference implementation.
template <typename GridT,
          typename IntersectorT = tools::LevelSetRayIntersector<GridT>>
...
public:
  ...
  LevelSetRayTracer(const GridT &grid, const BaseShader &shader,
                    BaseCamera &camera, size_t pixelSamples = 1,
                    unsigned int seed = 0);

  /// @brief Constructor based on an instance of the intersector
  /// performing the ray-intersections.
  LevelSetRayTracer(const IntersectorT &inter, const BaseShader &shader,
                    BaseCamera &camera, size_t pixelSamples = 1,
                    unsigned int seed = 0);
...
/// @brief A (very) simple multithreaded volume render specifically for scalar
/// density.
/// @details Included primarily as a reference implementation.
...
/// @brief A simple class that allows for concurrent writes to pixels in an
/// image, background initialization of the image, and PPM or EXR file output.
class Film {
public:
  ...
  struct RGBA {
    ...
    explicit RGBA(ValueT intensity)
        : r(intensity), g(intensity), b(intensity), a(1) {}
    RGBA(ValueT _r, ValueT _g, ValueT _b, ValueT _a = static_cast<ValueT>(1.0))
        : r(_r), g(_g), b(_b), a(_a) {}
    RGBA(double _r, double _g, double _b, double _a = 1.0)
        : r(static_cast<ValueT>(_r)), g(static_cast<ValueT>(_g)),
          b(static_cast<ValueT>(_b)), a(static_cast<ValueT>(_a)) {}

    RGBA operator*(ValueT scale) const {
      return RGBA(r * scale, g * scale, b * scale);
    }
    RGBA operator+(const RGBA &rhs) const {
      return RGBA(r + rhs.r, g + rhs.g, b + rhs.b);
    }
    RGBA operator*(const RGBA &rhs) const {
      return RGBA(r * rhs.r, g * rhs.g, b * rhs.b);
    }
  ...
  void checkerboard(const RGBA &c1 = RGBA(0.3f), const RGBA &c2 = RGBA(0.6f),
                    size_t size = 32) {
    RGBA *p = mPixels.get();
  ...
  void savePPM(const std::string &fileName) {
    ...
    os.write(reinterpret_cast<const char *>(&(*tmp)),
             3 * mSize * sizeof(unsigned char));
  }

#ifdef OPENVDB_TOOLS_RAYTRACER_USE_EXR
  void saveEXR(const std::string &fileName, size_t compression = 2,
               size_t threads = 8) {
    std::string name(fileName);
    if (name.find_last_of(".") == std::string::npos)
      name.append(".exr");
...
public:
  BaseCamera(Film &film, const Vec3R &rotation, const Vec3R &translation,
      ...
      : mFilm(&film), mScaleWidth(frameWidth),
        mScaleHeight(frameWidth * double(film.height()) /
                     double(film.width())) {
  ...
  virtual math::Ray<double> getRay(size_t i, size_t j, double iOffset = 0.5,
                                   double jOffset = 0.5) const = 0;
...
public:
  ...
  ///                     (the default of 50mm corresponds to Houdini's default
  ///                     camera)
  /// @param aperture     width in mm of the frame, i.e., the visible field
  ///                     (the default 41.2136 mm corresponds to Houdini's
  ///                     default camera)
  ...
  /// at (0,0,0) in world space and points in the direction of the negative z
  /// axis.
  PerspectiveCamera(Film &film, const Vec3R &rotation = Vec3R(0.0),
                    const Vec3R &translation = Vec3R(0.0),
                    double focalLength = 50.0, double aperture = 41.2136,
                    double nearPlane = 1e-3,
                    double farPlane = std::numeric_limits<double>::max())
      : BaseCamera(film, rotation, translation, 0.5 * aperture / focalLength,
                   nearPlane, farPlane) {}
  ...
  /// optional offsets in the range [0,1]. An offset of 0.5 corresponds
  /// to the center of the pixel.
  math::Ray<double> getRay(size_t i, size_t j, double iOffset = 0.5,
                           double jOffset = 0.5) const override {
    math::Ray<double> ray(mRay);
    Vec3R dir =
        BaseCamera::rasterToScreen(Real(i) + iOffset, Real(j) + jOffset, -1.0);
...
public:
  ...
  /// at (0,0,0) in world space and points in the direction of the negative z
  /// axis.
  OrthographicCamera(Film &film, const Vec3R &rotation = Vec3R(0.0),
                     const Vec3R &translation = Vec3R(0.0),
                     double frameWidth = 1.0, double nearPlane = 1e-3,
                     double farPlane = std::numeric_limits<double>::max())
      : BaseCamera(film, rotation, translation, 0.5 * frameWidth, nearPlane,
                   farPlane) {}
  ...
  math::Ray<double> getRay(size_t i, size_t j, double iOffset = 0.5,
    ...
    math::Ray<double> ray(mRay);
    Vec3R eye =
        BaseCamera::rasterToScreen(Real(i) + iOffset, Real(j) + jOffset, 0.0);
...
public:
  ...
  /// @brief Defines the interface of the virtual function that returns a RGB
  /// color.
  ...
  virtual Film::RGBA operator()(const Vec3R &xyz, const Vec3R &nml,
                                const Vec3R &dir) const = 0;
...
public:
  ...
  Film::RGBA operator()(const Vec3R &xyz, const Vec3R &,
                        const Vec3R &) const override {
  ...
  BaseShader *copy() const override {
    return new MatteShader<GridT, SamplerType>(*this);
  }
...
public:
  ...
  MatteShader(const MatteShader &) = default;
  ~MatteShader() override = default;
  Film::RGBA operator()(const Vec3R &, const Vec3R &,
                        const Vec3R &) const override {
    return mRGBA;
  }
  BaseShader *copy() const override {
    return new MatteShader<Film::RGBA, SamplerType>(*this);
  }
...
public:
  ...
  Film::RGBA operator()(const Vec3R &xyz, const Vec3R &normal,
                        const Vec3R &) const override {
    typename GridT::ValueType v = zeroVal<typename GridT::ValueType>();
    SamplerType::sample(mAcc, mXform->worldToIndex(xyz), v);
    return Film::RGBA(v[0] * (normal[0] + 1.0), v[1] * (normal[1] + 1.0),
                      v[2] * (normal[2] + 1.0));
  }
  BaseShader *copy() const override {
    return new NormalShader<GridT, SamplerType>(*this);
  }
...
public:
  ...
  NormalShader(const NormalShader &) = default;
  ~NormalShader() override = default;
  Film::RGBA operator()(const Vec3R &, const Vec3R &normal,
                        const Vec3R &) const override {
    return mRGBA *
           Film::RGBA(normal[0] + 1.0, normal[1] + 1.0, normal[2] + 1.0);
  }
  BaseShader *copy() const override {
    return new NormalShader<Film::RGBA, SamplerType>(*this);
  }
...
public:
  PositionShader(const math::BBox<Vec3R> &bbox, const GridT &grid)
      : mMin(bbox.min()), mInvDim(1.0 / bbox.extents()),
        mAcc(grid.getAccessor()), mXform(&grid.transform()) {}
  ...
  BaseShader *copy() const override {
    return new PositionShader<GridT, SamplerType>(*this);
  }
...
public:
  ...
  BaseShader *copy() const override {
    return new PositionShader<Film::RGBA, SamplerType>(*this);
  }
...
public:
  ...
  Film::RGBA operator()(const Vec3R &xyz, const Vec3R &normal,
                        const Vec3R &rayDir) const override {
    typename GridT::ValueType v = zeroVal<typename GridT::ValueType>();
    SamplerType::sample(mAcc, mXform->worldToIndex(xyz), v);
  ...
  BaseShader *copy() const override {
    return new DiffuseShader<GridT, SamplerType>(*this);
  }
...
public:
  ...
  BaseShader *copy() const override {
    return new DiffuseShader<Film::RGBA, SamplerType>(*this);
  }
...
template <typename GridT>
inline void rayTrace(const GridT &grid, const BaseShader &shader,
                     BaseCamera &camera, size_t pixelSamples, unsigned int seed,
                     bool threaded) {
  LevelSetRayTracer<GridT, tools::LevelSetRayIntersector<GridT>> tracer(
      grid, shader, camera, pixelSamples, seed);
  tracer.render(threaded);
}

template <typename GridT, typename IntersectorT>
inline void rayTrace(const GridT &, const IntersectorT &inter,
                     const BaseShader &shader, BaseCamera &camera,
                     size_t pixelSamples, unsigned int seed, bool threaded) {
  LevelSetRayTracer<GridT, IntersectorT> tracer(inter, shader, camera,
                                                pixelSamples, seed);
  tracer.render(threaded);
}
...
template <typename GridT, typename IntersectorT>
inline LevelSetRayTracer<GridT, IntersectorT>::LevelSetRayTracer(
    const GridT &grid, const BaseShader &shader, BaseCamera &camera,
    size_t pixelSamples, unsigned int seed)
    : mIsMaster(true), mRand(nullptr), mInter(grid), mShader(shader.copy()),
      mCamera(&camera) {
...
template <typename GridT, typename IntersectorT>
inline LevelSetRayTracer<GridT, IntersectorT>::LevelSetRayTracer(
    const IntersectorT &inter, const BaseShader &shader, BaseCamera &camera,
    size_t pixelSamples, unsigned int seed)
    : mIsMaster(true), mRand(nullptr), mInter(inter), mShader(shader.copy()),
      mCamera(&camera) {
  this->setPixelSamples(pixelSamples, seed);
}
...
template <typename GridT, typename IntersectorT>
inline LevelSetRayTracer<GridT, IntersectorT>::LevelSetRayTracer(
    const LevelSetRayTracer &other)
    : mIsMaster(false), mRand(other.mRand), mInter(other.mInter),
      mShader(other.mShader->copy()), mCamera(other.mCamera),
...
template <typename GridT, typename IntersectorT>
inline void LevelSetRayTracer<GridT, IntersectorT>::setGrid(const GridT &grid) {
  assert(mIsMaster);
  mInter = IntersectorT(grid);
}

template <typename GridT, typename IntersectorT>
inline void LevelSetRayTracer<GridT, IntersectorT>::setIntersector(
...
template <typename GridT, typename IntersectorT>
inline void
LevelSetRayTracer<GridT, IntersectorT>::setShader(const BaseShader &shader) {
  assert(mIsMaster);
...
template <typename GridT, typename IntersectorT>
inline void
LevelSetRayTracer<GridT, IntersectorT>::setCamera(BaseCamera &camera) {
  assert(mIsMaster);
  mCamera = &camera;
}
...
template <typename GridT, typename IntersectorT>
inline void
LevelSetRayTracer<GridT, IntersectorT>::setPixelSamples(size_t pixelSamples,
                                                        unsigned int seed) {
  assert(mIsMaster);
  if (pixelSamples == 0) {
  ...
  if (mSubPixels > 0) {
    mRand = new double[16];
    math::Rand01<double> rand(
        seed); // offsets for anti-aliaing by jittered super-sampling
...
template <typename GridT, typename IntersectorT>
inline void
LevelSetRayTracer<GridT, IntersectorT>::render(bool threaded) const {
  tbb::blocked_range<size_t> range(0, mCamera->height());
  threaded ? tbb::parallel_for(range, *this) : (*this)(range);
}

template <typename GridT, typename IntersectorT>
inline void LevelSetRayTracer<GridT, IntersectorT>::operator()(
  ...
  for (size_t j = range.begin(), n = 0, je = range.end(); j < je; ++j) {
    for (size_t i = 0, ie = mCamera->width(); i < ie; ++i) {
      ...
      Film::RGBA c =
          mInter.intersectsWS(ray, xyz, nml) ? shader(xyz, nml, ray.dir()) : bg;
      for (size_t k = 0; k < mSubPixels; ++k, n += 2) {
        ray = mCamera->getRay(i, j, mRand[n & 15], mRand[(n + 1) & 15]);
        c += mInter.intersectsWS(ray, xyz, nml) ? shader(xyz, nml, ray.dir())
                                                : bg;
...
inline VolumeRender<IntersectorT, SampleT>::VolumeRender(
    const IntersectorT &inter, BaseCamera &camera)
    : mAccessor(inter.grid().getConstAccessor()), mCamera(&camera),
      mPrimary(new IntersectorT(inter)), mShadow(new IntersectorT(inter)),
      mPrimaryStep(1.0), mShadowStep(3.0), mCutOff(0.005), mLightGain(0.2),
      mLightDir(Vec3R(0.3, 0.3, 0).unit()), mLightColor(0.7, 0.7, 0.7),
      mAbsorption(0.1), mScattering(1.5) {}

template <typename IntersectorT, typename SampleT>
inline VolumeRender<IntersectorT, SampleT>::VolumeRender(
    const VolumeRender &other)
    : mAccessor(other.mAccessor), mCamera(other.mCamera),
      mPrimary(new IntersectorT(*(other.mPrimary))),
      mShadow(new IntersectorT(*(other.mShadow))),
      mPrimaryStep(other.mPrimaryStep), mShadowStep(other.mShadowStep),
      mCutOff(other.mCutOff), mLightGain(other.mLightGain),
      mLightDir(other.mLightDir), mLightColor(other.mLightColor),
      mAbsorption(other.mAbsorption), mScattering(other.mScattering) {}

template <typename IntersectorT, typename SampleT>
inline void VolumeRender<IntersectorT, SampleT>::print(std::ostream &os,
                                                       int verboseLevel) {
  if (verboseLevel > 0) {
...
template <typename IntersectorT, typename SampleT>
inline void
VolumeRender<IntersectorT, SampleT>::setIntersector(const IntersectorT &inter) {
  mPrimary.reset(new IntersectorT(inter));
  mShadow.reset(new IntersectorT(inter));
}

template <typename IntersectorT, typename SampleT>
inline void VolumeRender<IntersectorT, SampleT>::render(bool threaded) const {
  tbb::blocked_range<size_t> range(0, mCamera->height());
  threaded ? tbb::parallel_for(range, *this) : (*this)(range);
}

template <typename IntersectorT, typename SampleT>
inline void VolumeRender<IntersectorT, SampleT>::operator()(
    const tbb::blocked_range<size_t> &range) const {
  SamplerType sampler(mAccessor,
                      mShadow->grid().transform()); // light-weight wrapper
  ...
  const Vec3R albedo = mLightColor * mScattering /
                       (mScattering + mAbsorption); // single scattering
  const Real sGain = mLightGain; // in-scattering along shadow ray
  const Real pStep =
      mPrimaryStep; // Integration step along primary ray in voxel units
  const Real sStep =
      mShadowStep; // Integration step along shadow ray in voxel units
        ...
        for (; pT <= pT1; pT += pStep) {
          ...
          for (size_t l = 0; l < sTS.size(); ++l) {
            ...
            for (; sT <= sT1; sT += sStep) {
              ...
              if (sTrans.lengthSqr() < cutoff)
                goto Luminance; // Terminate sRay
            }                   // Integration over shadow segment
          }                     // Shadow ray march
        Luminance:
          pLumi += albedo * sTrans * pTrans * (One - dT);
          pTrans *= dT;
          if (pTrans.lengthSqr() < cutoff)
            goto Pixel; // Terminate Ray
