...
struct RenderOpts {
  ...
  RenderOpts()
      : shader("diffuse"), camera("perspective"), aperture(41.2136f),
        focal(50.0f), frame(1.0f), znear(1.0e-3f),
        zfar(std::numeric_limits<float>::max()), isovalue(0.0), rotate(0.0),
        translate(0.0), target(0.0), up(0.0, 1.0, 0.0), lookat(false),
        samples(1), absorb(0.1), light(LIGHT_DEFAULTS, LIGHT_DEFAULTS + 6),
        scatter(1.5), cutoff(0.005), gain(0.2), step(1.0, 3.0), width(1920),
        height(1080), compression("zip"), threads(0), verbose(false) {}

  std::string validate() const {
    if (shader != "diffuse" && shader != "matte" && shader != "normal" &&
        shader != "position") {
      return "expected diffuse, matte, normal or position shader, got \"" +
             shader + "\"";
    }
    if (!boost::starts_with(camera, "ortho") &&
        !boost::starts_with(camera, "persp")) {
    ...
    if (width < 1 || height < 1) {
      ...
      ostr << "expected width > 0 and height > 0, got " << width << "x"
           << height;
      return ostr.str();
    }
    return "";
  ...
  std::ostream &put(std::ostream &os) const {
    os << " -absorb " << absorb[0] << "," << absorb[1] << "," << absorb[2]
       << " -aperture " << aperture << " -camera " << camera;
    if (!color.empty())
      os << " -color '" << color << "'";
    os << " -compression " << compression << " -cpus " << threads << " -cutoff "
       << cutoff << " -far " << zfar << " -focal " << focal << " -frame "
       << frame << " -gain " << gain << " -isovalue " << isovalue << " -light "
       << light[0] << "," << light[1] << "," << light[2] << "," << light[3]
       << "," << light[4] << "," << light[5];
    if (lookat)
      os << " -lookat " << target[0] << "," << target[1] << "," << target[2];
    os << " -near " << znear << " -res " << width << "x" << height;
    if (!lookat)
      os << " -rotate " << rotate[0] << "," << rotate[1] << "," << rotate[2];
    os << " -shader " << shader << " -samples " << samples << " -scatter "
       << scatter[0] << "," << scatter[1] << "," << scatter[2]
       << " -shadowstep " << step[1] << " -step " << step[0] << " -translate "
       << translate[0] << "," << translate[1] << "," << translate[2];
    if (lookat)
      os << " -up " << up[0] << "," << up[1] << "," << up[2];
    if (verbose)
      os << " -v";
    return os;
  }
...
std::ostream &operator<<(std::ostream &os, const RenderOpts &opts) {
  return opts.put(os);
}
...
void usage [[noreturn]] (int exitStatus = EXIT_FAILURE) {
  ...
  const double fov =
      openvdb::tools::PerspectiveCamera::focalLengthToFieldOfView(
          opts.focal, opts.aperture);

  std::ostringstream ostr;
  ostr
      << std::setprecision(3) << "Usage: " << gProgName
      << " in.vdb out.{exr,ppm} [options]\n"
      << "Which: ray-traces OpenVDB volumes\n"
      << "Options:\n"
      << "    -aperture F       perspective camera aperture in mm (default: "
      << opts.aperture << ")\n"
      << "    -camera S         camera type; either \"persp[ective]\" or "
         "\"ortho[graphic]\"\n"
      << "                      (default: " << opts.camera << ")\n"
      << "    -compression S    EXR compression scheme; either \"none\" "
         "(uncompressed),\n"
      << "                      \"rle\" or \"zip\" (default: "
      << opts.compression << ")\n"
      << "    -cpus N           number of rendering threads, or 1 to disable "
         "threading,\n"
      << "                      or 0 to use all available CPUs (default: "
      << opts.threads << ")\n"
      << "    -far F            camera far plane depth (default: " << opts.zfar
      << ")\n"
      << "    -focal F          perspective camera focal length in mm "
         "(default: "
      << opts.focal << ")\n"
      << "    -fov F            perspective camera field of view in degrees\n"
      << "                      (default: " << fov << ")\n"
      << "    -frame F          ortho camera frame width in world units "
         "(default: "
      << opts.frame << ")\n"
      << "    -lookat X,Y,Z     rotate the camera to point to (X, Y, Z)\n"
      << "    -name S           name of the volume to be rendered (default: "
         "render\n"
      << "                      the first floating-point volume found in "
         "in.vdb)\n"
      << "    -near F           camera near plane depth (default: "
      << opts.znear << ")\n"
      ...
      << "    -up X,Y,Z         vector that should point up after rotation "
         "with -lookat\n"
      << "                      (default: " << opts.up << ")\n"
      << "\n"
      ...
      << "\n"
      << "Level set options:\n"
      << "    -color S          name of a vec3s volume to be used to set "
         "material colors\n"
      << "    -isovalue F       isovalue in world units for level set ray "
         "intersection\n"
      << "                      (default: " << opts.isovalue << ")\n"
      << "    -samples N        number of samples (rays) per pixel (default: "
      << opts.samples << ")\n"
      << "    -shader S         shader name; either \"diffuse\", \"matte\", "
         "\"normal\"\n"
      << "                      or \"position\" (default: " << opts.shader
      << ")\n"
      << "\n"
      << "Dense volume options:\n"
      << "    -absorb R,G,B     absorption coefficients (default: "
      << opts.absorb << ")\n"
      << "    -cutoff F         density and transmittance cutoff value "
         "(default: "
      << opts.cutoff << ")\n"
      << "    -gain F           amount of scatter along the shadow ray "
         "(default: "
      << opts.gain << ")\n"
      << "    -light X,Y,Z[,R,G,B]  light source direction and optional color\n"
      << "                      (default: [" << opts.light[0] << ", "
      << opts.light[1] << ", " << opts.light[2] << ", " << opts.light[3] << ", "
      << opts.light[4] << ", " << opts.light[5] << "])\n"
      << "    -scatter R,G,B    scattering coefficients (default: "
      << opts.scatter << ")\n"
      << "    -shadowstep F     step size in voxels for integration along the "
         "shadow ray\n"
      << "                      (default: " << opts.step[1] << ")\n"
      << "    -step F           step size in voxels for integration along the "
         "primary ray\n"
      << "                      (default: " << opts.step[0] << ")\n"
      << "\n"
      << "Examples:\n"
      << "    " << gProgName
      << " crawler.vdb crawler.exr -shader diffuse -res 1920x1080 \\\n"
      << "        -focal 35 -samples 4 -translate 0,210.5,400 -compression rle "
         "-v\n"
      << "\n"
      << "    " << gProgName
      << " bunny_cloud.vdb bunny_cloud.exr -res 1920x1080 \\\n"
      << "        -translate 0,0,110 -absorb 0.4,0.2,0.1 -gain 0.2 -v\n"
      << "\n"
      << "Warning:\n"
      << "     This is not (and is not intended to be) a production-quality "
         "renderer.\n"
      << "     Use it for fast previewing or simply as a reference "
         "implementation\n"
...
void saveEXR(const std::string &fname, const openvdb::tools::Film &film,
             const RenderOpts &opts) {
  using RGBA = openvdb::tools::Film::RGBA;

  std::string filename = fname;
  if (!boost::iends_with(filename, ".exr"))
    filename += ".exr";
  ...
  if (opts.compression == "none") {
    ...
    OPENVDB_THROW(openvdb::ValueError,
                  "expected none, rle or zip compression, got \""
                      << opts.compression << "\"");
  }
  ...
  framebuffer.insert("R",
                     Imf::Slice(Imf::FLOAT, reinterpret_cast<char *>(&pixel0.r),
                                pixelBytes, rowBytes));
  framebuffer.insert("G",
                     Imf::Slice(Imf::FLOAT, reinterpret_cast<char *>(&pixel0.g),
                                pixelBytes, rowBytes));
  framebuffer.insert("B",
                     Imf::Slice(Imf::FLOAT, reinterpret_cast<char *>(&pixel0.b),
                                pixelBytes, rowBytes));
  framebuffer.insert("A",
                     Imf::Slice(Imf::FLOAT, reinterpret_cast<char *>(&pixel0.a),
                                pixelBytes, rowBytes));
...
void render(const GridType &grid, const std::string &imgFilename,
            const RenderOpts &opts) {
  ...
  if (boost::starts_with(opts.camera, "persp")) {
    camera.reset(new tools::PerspectiveCamera(film, opts.rotate, opts.translate,
                                              opts.focal, opts.aperture,
                                              opts.znear, opts.zfar));
  } else if (boost::starts_with(opts.camera, "ortho")) {
    camera.reset(new tools::OrthographicCamera(
        film, opts.rotate, opts.translate, opts.frame, opts.znear, opts.zfar));
  } else {
    OPENVDB_THROW(ValueError,
                  "expected perspective or orthographic camera, got \""
                      << opts.camera << "\"");
  }
  if (opts.lookat)
    camera->lookAt(opts.target, opts.up);

  // Define the shader for level set rendering.  The default shader is a diffuse
  // shader.
  ...
  if (opts.shader == "matte") {
    ...
    const math::BBox<Vec3d> bboxIndex(bbox.min().asVec3d(),
                                      bbox.max().asVec3d());
    const math::BBox<Vec3R> bboxWorld =
        bboxIndex.applyMap(*(grid.transform().baseMap()));
    if (opts.colorgrid) {
      shader.reset(
          new tools::PositionShader<Vec3SGrid>(bboxWorld, *opts.colorgrid));
    } else {
  ...
  if (boost::iends_with(imgFilename, ".ppm")) {
    ...
    OPENVDB_THROW(ValueError,
                  "unsupported image file format (" + imgFilename + ")");
  }
}

void strToSize(const std::string &s, size_t &x, size_t &y) {
  std::vector<std::string> elems;
  boost::split(elems, s, boost::algorithm::is_any_of(",x"));
  const size_t numElems = elems.size();
  if (numElems > 0)
    x = size_t(std::max(0, atoi(elems[0].c_str())));
  if (numElems > 1)
    y = size_t(std::max(0, atoi(elems[1].c_str())));
}

std::vector<double> strToVec(const std::string &s) {
...
openvdb::Vec3d strToVec3d(const std::string &s) {
...
struct OptParse {
  ...
  bool check(int idx, const std::string &name, int numArgs = 1) const {
    if (argv[idx] == name) {
      if (idx + numArgs >= argc) {
        OPENVDB_LOG_FATAL("option " << name << " requires " << numArgs
                                    << " argument"
                                    << (numArgs == 1 ? "" : "s"));
        usage();
      }
...
int main(int argc, char *argv[]) {
  ...
  gProgName = argv[0];
  if (const char *ptr = ::strrchr(gProgName, '/'))
    gProgName = ptr + 1;
  ...
  if (hasFov) {
    ...
    opts.focal =
        float(openvdb::tools::PerspectiveCamera::fieldOfViewToFocalLength(
            fov, opts.aperture));
  }
  ...
  try {
    tbb::task_scheduler_init schedulerInit(
        (opts.threads == 0) ? tbb::task_scheduler_init::automatic
                            : opts.threads);
    ...
    openvdb::FloatGrid::Ptr grid;
    {
      ...
      if (!gridName.empty()) {
        file.open();
        grid =
            openvdb::gridPtrCast<openvdb::FloatGrid>(file.readGrid(gridName));
        if (!grid) {
          OPENVDB_THROW(openvdb::ValueError,
                        gridName + " is not a scalar, floating-point volume");
        }
      } else {
        // If no grid was specified by name, retrieve the first float grid from
        // the file.
        ...
        for (size_t i = 0; i < grids->size(); ++i, ++it) {
          ...
          if (grid) {
            ...
            file.close();
            file.open();
            grid = openvdb::gridPtrCast<openvdb::FloatGrid>(
        ...
        if (!grid) {
          OPENVDB_THROW(openvdb::ValueError,
                        "no scalar, floating-point volumes in file " +
                            vdbFilename);
        }
      }
